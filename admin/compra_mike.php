...
foreach ($_POST['asiento'] as $asientos){
	$result = $con->query("SELECT id, status FROM morelos where seccion = '".$seccion."' and fila = '".$fila."' and asiento = ".$asientos);
	if ($row = mysqli_fetch_array($result)){
		if($row["status"] == 1){
			header('Location: form_compra_mike.php?error=ocupado');
		}
	}
}
...
foreach ($_POST['asiento'] as $asientos){
	mysqli_query($con, "UPDATE morelos set status = 1, confirmacion = 1, forma_pago = '".$forma_pago."', folio = ".$folio.", user = '".$_SESSION["id"]."' where seccion = '".$seccion."' and fila = '".$fila."' and asiento = ".$asientos);
